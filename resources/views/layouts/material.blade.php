...
<div class="relative min-h-screen bg-gray-50">
    ...
    <div class="max-w-3xl mx-auto p-4 sm:p-8">
        ...
        <div class="fixed bottom-0 left-0 w-full bg-white/95 shadow-[0_-2px_12px_-2px_rgba(0,0,0,0.09)] flex items-center justify-between px-4 py-3 z-30
                    sm:static sm:bg-transparent sm:shadow-none sm:px-0 sm:py-0 mt-12 gap-4">
            @if ($previous)
                <a href="{{ route('materi.show', $previous->slug) }}"
                class="px-4 py-2 rounded-xl bg-slate-200 text-slate-700 hover:bg-slate-300 font-semibold transition w-full sm:w-auto text-center">
                    &larr; Materi Sebelumnya
                </a>
            @else
                <span class="px-4 py-2 rounded-xl bg-slate-100 text-slate-400 cursor-not-allowed w-full sm:w-auto text-center">&larr; Materi Sebelumnya</span>
            @endif

            @if ($material->is_submission_point && !$next)
                <a href="{{ route('materials.index') }}"
                class="px-4 py-2 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 font-semibold transition w-full sm:w-auto text-center">
            ...
            @else
                <span class="px-4 py-2 rounded-xl bg-slate-100 text-slate-400 cursor-not-allowed w-full sm:w-auto text-center">Materi Selanjutnya &rarr;</span>
            @endif
        </div>
